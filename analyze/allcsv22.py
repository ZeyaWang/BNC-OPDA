...
for f in os.listdir(fd):
    if f.endswith('.csv'):
        ...
        if len(f2[:-4].split('_')) == 15+1:
            _, _, domain, src, tar, balance, lr, lr_scale, interval, lambdav, max_k, KK, cov, sc, clf, bs = f2[:-4].split('_')
            if domain == ttype:
                myd[(balance, lr, lr_scale, interval, lambdav, KK, cov, sc, clf, bs, max_k)].append(
                    [src, tar, hos, acc_test, nmi, k_acc, uk_nmi, epoch])
        elif len(f2[:-4].split('_')) == 16+1:
            _, _, domain, src, tar, balance, lr, lr_scale, interval, lambdav, max_k, KK, cov, sc, clf, bs, dt = f2[:-4].split('_')
            if domain == ttype:
                myd[(balance, lr, lr_scale, interval, lambdav, KK, cov, sc, clf, bs, dt, max_k)].append(
                    [src, tar, hos, acc_test, nmi, k_acc, uk_nmi, epoch])
        elif len(f2[:-4].split('_')) == 14+1:
            _, _, domain, src, tar, balance, lr, lr_scale, interval, lambdav, max_k, KK, cov, sc, clf = f2[:-4].split('_')
            if domain == ttype:
                myd[(balance, lr, lr_scale, interval, lambdav, KK, cov, sc, clf, max_k)].append(
                    [src, tar, hos, acc_test, nmi, k_acc, uk_nmi, epoch])
        elif len(f2[:-4].split('_')) == 13+1:
            _, _, src, tar, balance, lr, lr_scale, interval, lambdav, max_k, KK, cov, sc, clf = f2[:-4].split('_')
            myd[(balance, lr, lr_scale, interval, lambdav, KK, cov, sc, clf, max_k)].append(
                [src, tar, hos, acc_test, nmi, k_acc, uk_nmi, epoch])
...
for i in epoch:
    ...
    for f in os.listdir(fd):
        if f.endswith('.csv'):
            ...
            # if (balance == '0.01'):# and (KK != '5'):
            if len(f2[:-4].split('_')) == 15+1:
                _, _, domain, src, tar, balance, lr, lr_scale, interval, lambdav, max_k, KK, cov, sc, clf, bs = f2[
                                                                                                             :-4].split(
                    '_')
                if domain == ttype:
                    myd[(balance, lr, lr_scale, interval, lambdav, KK, cov, sc, clf, bs, max_k)].append(
                ...
                _, _, domain, src, tar, balance, lr, lr_scale, interval, lambdav, max_k, KK, cov, sc, clf, bs, dt = f2[
                                                                                                                 :-4].split(
                    '_')
                if domain == ttype:
                    myd[(balance, lr, lr_scale, interval, lambdav, KK, cov, sc, clf, bs, dt, max_k)].append(
                ...
                _, _, domain, src, tar, balance, lr, lr_scale, interval, lambdav, max_k, KK, cov, sc, clf = f2[:-4].split(
                    '_')
                if domain == ttype:
                    myd[(balance, lr, lr_scale, interval, lambdav, KK, cov, sc, clf, max_k)].append(
                        [src, tar, hos, acc_test, nmi, k_acc, uk_nmi, epoch])
